...
- 可配置任意 OpenAI `chat/completions` 兼容接口、模型名称、API Key、自定义请求参数和请求超时时间。
...
3. 点击面板右上角的设置按钮，填写：
   ...
   - 模型名称：服务商支持的模型 ID。
   - API Key：对应服务商签发的密钥。
   - 自定义额外参数：会作为 JSON 对象合并到请求体中，同名字段以这里填写的值为准。默认值为 `{"thinking": { "type": "disabled" }}`；如果当前服务商不支持该参数，可改成 `{}`，或按照其文档填写 `reasoning_effort`、`enable_thinking` 等参数。
   - 请求超时时间：正式分析和接口测试共用，默认 120 秒。
4. 点击“测试”确认接口可用。
5. 返回主面板，开始过滤。
